...
kubectl storageos install --dry-run --k8s-version=v1.22.0 --etcd-endpoints=storageos.etcd:2379 --stos-version=develop --etcd-operator-version=develop --enable-metrics=true
cp -r storageos-dry-run "$TMPDIR/" && rm -r storageos-dry-run
grep -A 1 '  metrics:$' "$TMPDIR"/storageos-dry-run/*storageos-cluster.yaml | tail -n 1 | diff <(printf '    enabled: true\n') -
mv "$TMPDIR"/storageos-dry-run/*storageos-cluster.yaml "$TMP_STOS_CLUSTER"
# make sure that the metrics-enabled settings are preserved
kubectl storageos install --dry-run --k8s-version=v1.22.0 --etcd-endpoints=storageos.etcd:2379 --stos-version=develop --etcd-operator-version=develop --stos-cluster-yaml="$TMP_STOS_CLUSTER"
cp -r storageos-dry-run "$TMPDIR/" && rm -r storageos-dry-run
grep -A 1 '  metrics:$' "$TMPDIR"/storageos-dry-run/*storageos-cluster.yaml | tail -n 1 | diff <(printf '    enabled: true\n') -
kubectl storageos install --dry-run --k8s-version=v1.22.0 --etcd-endpoints=storageos.etcd:2379 --stos-version=develop --etcd-operator-version=develop --stos-cluster-yaml="$TMP_STOS_CLUSTER" --enable-metrics=false
cp -r storageos-dry-run "$TMPDIR/" && rm -r storageos-dry-run
# make sure the --enable-metrics flag overrides the config
grep -A 1 '  metrics:$' "$TMPDIR"/storageos-dry-run/*storageos-cluster.yaml | tail -n 1 | diff <(printf '    enabled: false\n') -
